...
### 8.6 PCA (QC6) sempre diagnóstica, mas opcional no critério de QF

Pedido em 2026-06-28, registrado no README ("Roadmap") — **ainda não implementado**. Hoje `Score_PCA` está sempre incluído incondicionalmente em `QI_WEIGHTS` (peso 0.05) — não há nenhum jeito de excluir a PCA do cálculo de `QF` mantendo-a só como diagnóstico visual. A ideia é um checkbox na sidebar: PCA continua sempre calculada e exibida na aba de diagnóstico (isso não muda), mas sua contribuição ao `QI`/`QF` passaria a ser opcional.
