...
Agregação:
- `compute_scores(rep0, strict_missing_data=True, combine_rolling_vars=False)` — combina os scores individuais em `QI` via `QI_WEIGHTS` (pesos nomeados: 0.25/0.15/0.20/0.20/0.15/0.05) e calcula percentis 95/99 de Mahalanobis. `strict_missing_data` controla como o `QI` é tratado quando algum score é `NaN`: restritivo (padrão) → `QI=NaN` (qualquer score ausente invalida a linha); flexível → `QI` recalculado só com os módulos disponíveis, redistribuindo os pesos. `combine_rolling_vars` controla o `Score_Rolling`/`rep0["Rolling_z"]` do QC4: padrão (`False`) → só `Rh-La-Inc Area_delta_z` (dado espectral real); opt-in (`True`) → maior `|delta_z|` entre `ROLLING_VARS`.
