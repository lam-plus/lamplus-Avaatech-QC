...
- `qc_rolling` (QC4) — média móvel (`ROLLING_WINDOW`) + delta + z-score do delta, para `ROLLING_VARS` (`Throughput`, `Rh-La Area`, `Rh-La-Inc Area`). **Resolvido em 2026-06-28** (`TODO.md` item 4.1): as três são indicadores instrumentais/físicos da medição (não composição química) e tendem a reagir juntas a um mesmo problema físico (rachadura, bolha de ar, transição seco/úmido) — por isso `compute_scores` combina as três via `rep0["Rolling_z"] = max(|delta_z|)` por padrão (`combine_rolling_vars=True`), em vez de usar só `Rh-La-Inc Area_delta_z` como antes. Ver README.md ("About QC4") para a justificativa física completa.
...
Único arquivo de UI. Importa de `qc_core` (`CORE_DEPTH_COL`, `DEPTH_COL`, `QF_INDETERMINATE`, `QF_PLOT_ORDER`, `add_pointwise_flag_notes`, `check_file`, `run_qc`) e de `i18n` (`TEXTS`, `DEFAULT_LANG`). Define `QF_COLORS` no topo do arquivo (cor de cada QF no gráfico de barras; `QF_PLOT_ORDER` mora em `qc_core.py`, não aqui). Três controles na sidebar: seletor `DEPTH_DISPLAY_COL` (`T["depth_display_label"]`, default `DEPTH_COL`/CompositeDepth — **puramente de exibição**, ver `qc_core.CORE_DEPTH_COL`) e dois checkboxes que controlam `compute_scores`: `strict_missing_data` (default `True`) e `combine_rolling_vars` (default `True`). Estrutura:
...
4. ~~QC4 (rolling) calculava deltas para 3 variáveis mas só usava 1 no score/flag final~~ — **corrigido em 2026-06-28**: combina as três por padrão (`combine_rolling_vars`), com toggle para o legado; README atualizado.
