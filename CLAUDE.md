...
Único arquivo de UI. Importa de `qc_core` (`DEPTH_COL`, `QF_INDETERMINATE`, `check_file`, `run_qc`) e de `i18n` (`TEXTS`, `DEFAULT_LANG`). Define `QF_PLOT_ORDER`/`QF_COLORS` no topo do arquivo (posição visual e cor de cada QF no gráfico de barras — `QF_INDETERMINATE=9` é plotado numa posição própria, não na sua altura literal, para não distorcer a escala 0-3). Dois checkboxes na sidebar controlam o comportamento de `compute_scores` descrito acima: `strict_missing_data` (`T["strict_missing_label"]`, default `True`) e `combine_rolling_vars` (`T["combine_rolling_label"]`, default `True`). Estrutura:
