...
data/
  exemplo_dados_consolidados.xlsx   # Arquivo de exemplo real (aba única "10kv"), usado para testes manuais
  Dados Consolidados-ICCE3.xlsx     # Arquivo real MULTI-ENERGIA (3 abas: 10kV/30kV/50kV) — usado para validar suporte multi-energia (ver qc_core.ENERGY_PARAMETERS)
...
**Estrutura multi-energia (desde 2026-07-28, protocolo v4.2, `TODO.md` seção 9):** um workbook Avaatech pode ter uma aba por energia de tubo de raio-X (10 kV/30 kV/50 kV), cada uma medindo um subconjunto diferente de parâmetros — confirmado contra `data/Dados Consolidados-ICCE3.xlsx` (3 abas reais: 10 kV mede Rh-La Area/Rh-La-Inc Area/Ar-Ka Area; 30 kV mede Rh-Ka-Coh Area/Rh-Ka-Inc Area, sem Argônio; 50 kV não mede nenhum dos dois, só Throughput + elementos traço). `ENERGY_PARAMETERS` (dict `{"10kV"/"30kV"/"50kV": {"throughput", "argon", "coherent", "incoherent"}}`) espelha `config.py` do apêndice v4.2; `DEFAULT_ENERGY = "10kV"`. `detect_energy(sheet_name)` detecta a energia pelo nome da aba (substring "10"/"30"/"50", case-insensitive — reproduz `io_module.py` do apêndice), levanta `ValueError` para nomes não reconhecidos. `read_workbook(file_or_buffer)` lê todas as abas de um workbook e detecta a energia de cada uma; abas com energia não reconhecida são ignoradas (retornadas em `skipped`), exceto quando o workbook tem só uma aba — nesse caso assume `DEFAULT_ENERGY` (compatibilidade com exports mais antigos cujo nome de aba não segue a convenção).

Praticamente todas as funções do pipeline abaixo (`qc_throughput`, `qc_rh_la`, `qc_rh_la_inc`, `qc_rolling`, `compute_scores`, `compute_flags`, `check_file`, `run_qc`) ganharam um parâmetro `energy=DEFAULT_ENERGY` opcional — chamá-las sem especificar energia reproduz o comportamento anterior a esta mudança byte a byte (verificado por construção: `ENERGY_PARAMETERS["10kV"]` usa exatamente os mesmos literais já hardcoded antes). `REQUIRED_COLUMNS`/`CRITICAL_INPUT_COLS`/`ROLLING_VARS` (as constantes de módulo, para a energia padrão) agora são derivadas de `ENERGY_PARAMETERS` via `_required_columns_for_energy`/`_critical_cols_for_energy`/`_rolling_vars_for_energy`, em vez de literais soltos duplicando a mesma informação — chamar essas funções com `energy="30kV"`/`"50kV"` dá o equivalente específico daquela energia.

Um módulo QC2/QC3 (coerente/incoerente) estruturalmente ausente numa energia (ex. Rh-Lα/Rh-Lα-Inc em 50 kV) é tratado como **ausência estrutural do dataset inteiro**, não como *dado faltante pontual* (achado C2) — distinção importante: `compute_scores` força o score correspondente a 100 (neutro, mesmo padrão de `Score_PCA`/`Score_Replica`) e exclui seu peso do `QI` (renormalizado), em vez de deixar `QF_INDETERMINATE` disparar em toda a aba. QC4 (rolling) usa por padrão a variável incoerente da energia; quando ela não existe (50 kV), cai automaticamente no modo combinado (usa o que estiver disponível — na prática, só Throughput).
...
Validação:
- `check_file(df, lang="pt", energy=DEFAULT_ENERGY)` — valida estrutura do DataFrame **antes** de rodar o pipeline. Opera sobre uma única aba/energia por chamada — num workbook multi-energia, o chamador invoca uma vez por aba (ver `qc_avaatech.py`). Retorna `(errors, warnings)`. `errors` não-vazio bloqueia execução no frontend; `warnings` só avisa. Mensagens vêm de `CHECK_MESSAGES[key][lang]` (i18n.py), com templates `.format(**kwargs)`.

Módulos QC individuais (cada um recebe/retorna um DataFrame `rep0`, o subconjunto filtrado para `Replicate Nr Count == "Rep0"`):
- `qc_throughput` (QC1) — z-score robusto do Throughput combinado com Argônio (`ARGON_COL = "Ar-Ka Area"`, opcional — só existe em modo 10 kV). **Desde 2026-07-28** (protocolo v4.2, `TODO.md` seção 9 item b): `rep0["Instrument_z"]` = o z-score de maior magnitude absoluta entre `Throughput_z` e `Argon_z`, linha a linha ("pior dos dois" — vedação/atmosfera pode degradar só o Argônio sem derrubar o Throughput, e vice-versa); é `Instrument_z`, não `Throughput_z` isoladamente, que alimenta `Score_Throughput`/`compute_flags`. Se `Ar-Ka Area` não existir no arquivo, `Argon_z` fica `NaN` e `Instrument_z` cai de volta para `Throughput_z` puro (comportamento idêntico ao anterior a 2026-07-28).
- `qc_rh_la` (QC2), `qc_rh_la_inc` (QC3) — z-score robusto pontual.
- `qc_rolling` (QC4) — média móvel (`ROLLING_WINDOW`) + delta + z-score do delta, para `ROLLING_VARS` (`Throughput`, `Rh-La Area`, `Rh-La-Inc Area`, e **desde 2026-07-28** `Ar-Ka Area`). Ignora graciosamente qualquer variável de `ROLLING_VARS` ausente do arquivo (mesmo padrão de `qc_pca`/`qc_replicates`) — relevante para quando a estrutura multi-energia (30/50 kV, sem Argônio) for suportada. **Resolvido em 2026-06-28** (`TODO.md` item 4.1, revisado no mesmo dia): por padrão (`combine_rolling_vars=False`), `compute_scores` usa só `Rh-La-Inc Area_delta_z` — é o dado espectral real medido; Throughput/Rh-Lα/Argônio são parâmetros instrumentais secundários. Opt-in (`combine_rolling_vars=True`) combina as variáveis presentes via `rep0["Rolling_z"] = max(|delta_z|)`, já que um problema físico de medição (rachadura, bolha de ar, transição seco/úmido) tende a reagir em pelo menos uma delas simultaneamente — mais sensível, às custas de também reagir a deriva instrumental não necessariamente no dado espectral. Ver README.md ("About QC4") para a justificativa física completa. **Desde 2026-07-28** (protocolo v4.2, "Persistência" — ver `TODO.md` seção "Bloqueados até decisão do time"), `qc_rolling(rep0, use_rolling_persistence=False)`: opt-in (default preserva o comportamento anterior — qualquer ponto isolado acima do limiar já dispara o critério de deriva, "defesa em profundidade"). Quando `True`, cada `{var}_delta_z` passa por `_apply_rolling_persistence` antes de seguir no pipeline: um ponto com `|delta_z| > ROLLING_PERSISTENCE_Z_THRESHOLD` (2.0, o mesmo limiar de "atenção" já usado para Instrument_z/RhLa_z/RhLaInc_z) só mantém seu z-score se houver `>= ROLLING_PERSISTENCE_MIN_POINTS` (2) pontos acima do limiar numa janela de `ROLLING_PERSISTENCE_WINDOW` (3) medidas centrada nele — anomalias isoladas são zeradas (nunca pontos já abaixo do limiar, que ficam intocados). Como `compute_scores`/`compute_flags` sempre leem as colunas produzidas por `qc_rolling`, o efeito se propaga automaticamente para `Score_Rolling` e para o critério de flag do QC4 nos dois modos de QF, sem exigir parâmetro próprio nessas duas funções.
...
- `run_qc(df, strict_missing_data=True, combine_rolling_vars=False, include_pca_in_qf=False, use_count_mode=False, use_rolling_persistence=False, energy=DEFAULT_ENERGY)` — orquestra o pipeline completo sobre UMA aba/energia, retorna `(rep0, p95, p99, pca_elements)`. Num workbook multi-energia, chamar uma vez por aba com o `energy` detectado (ver `read_workbook`).

### `qc_avaatech.py` — frontend Streamlit

Único arquivo de UI. Importa de `qc_core` (`CORE_DEPTH_COL`, `DEFAULT_ENERGY`, `DEPTH_COL`, `ENERGY_PARAMETERS`, `QF_INDETERMINATE`, `QF_PLOT_ORDER`, `add_pointwise_flag_notes`, `check_file`, `read_workbook`, `run_qc`) e de `i18n` (`TEXTS`, `DEFAULT_LANG`). Define `QF_COLORS` no topo do arquivo (cor de cada QF no gráfico de barras; `QF_PLOT_ORDER` mora em `qc_core.py`, não aqui). Seis controles na sidebar: seletor `DEPTH_DISPLAY_COL` (`T["depth_display_label"]`, default `DEPTH_COL`/CompositeDepth — **puramente de exibição**, ver `qc_core.CORE_DEPTH_COL`) e cinco checkboxes que controlam `run_qc`/`compute_scores`/`compute_flags`: `strict_missing_data` (default `True`), `combine_rolling_vars` (default `False` — QC4 considera só Rh-Lα-Inc por padrão; combinar as 3 variáveis é opt-in), `include_pca_in_qf` (default `False` — PCA é só diagnóstico, não entra no QI/QF por padrão; opt-in reativa), `use_count_mode` (default `False` — QF por QI ponderado; opt-in troca para o modo de contagem de módulos do protocolo v4.2) e `use_rolling_persistence` (default `False` — QC4 mantém "defesa em profundidade", flag pontual único válido; opt-in aplica a regra de persistência do protocolo v4.2, ver `qc_core.qc_rolling`). Estrutura:
- Funções `plot_*(rep0, T, depth_col=DEPTH_COL)` — uma por aba de diagnóstico (Throughput, Rh-Lα/Rh-Lα-Inc, Rolling, Réplicas, PCA, QI/QF). `depth_col` é repassado como `DEPTH_DISPLAY_COL` na chamada — só muda o eixo X, nunca o cálculo. **Desde 2026-07-28** (multi-energia), `plot_rh`/`plot_rolling` também recebem `energy`: `plot_rh` usa as colunas físicas corretas por energia (via `ENERGY_PARAMETERS`) e retorna `None` quando a energia não mede nenhum dos dois parâmetros (50 kV — a UI mostra `T["rh_not_applicable_info"]` nesse caso); `plot_rolling` plota a mesma variável que alimenta `Rolling_z` por padrão em `compute_scores` (com fallback a Throughput em 50 kV) e usa `T["plot_rolling_title"]` como template (`{var}`, rótulo curto via `ROLLING_VAR_LABELS`) — para 10 kV renderiza texto idêntico ao anterior a esta mudança.
- `to_excel_bytes(sheet_results)` — serializa **todas** as abas processadas com sucesso num único workbook de download, uma aba por energia, preservando o nome original de cada aba e suas colunas originais (nunca achatando/combinando abas); `sheet_results` é a lista de dicts montada no corpo principal (`sheet_name`, `energy`, `df_raw`, `rep0`, `p95`, `p99`, `pca_elements`).
- **Multi-energia (desde 2026-07-28):** o corpo principal lê o workbook inteiro via `read_workbook(uploaded)` (detecta a energia de cada aba pelo nome), chama `check_file`/`run_qc` **uma vez por aba** — uma aba com erro é reportada e pulada, sem impedir as demais de aparecer. Um seletor (`st.selectbox`, chave `T["energy_sheet_selector_label"]`) só é exibido quando há mais de uma aba processada com sucesso, para não alterar a experiência de arquivos de aba única (ex. `data/exemplo_dados_consolidados.xlsx`). Abas cuja energia não é reconhecida pelo nome são ignoradas com aviso (`T["workbook_skipped_warning"]`), exceto quando o workbook tem só uma aba (assume `DEFAULT_ENERGY`, compatibilidade com exports mais antigos).
- Corpo principal roda no nível de módulo (padrão Streamlit): `st.set_page_config` → seletor de idioma → seletor de profundidade exibida → checkboxes de QC → upload → `read_workbook` → (`check_file` + `run_qc` + `add_pointwise_flag_notes`) por aba → seletor de aba/energia (se >1) → resumo em métricas → 6 abas de gráficos → tabela completa (`st.dataframe`, colunas reordenadas com `DEPTH_DISPLAY_COL` primeiro; nomes de coluna originais do instrumento não traduzidos) → botão de download (workbook com todas as abas processadas).
- **Não há `@st.cache_data`** em `check_file`/`run_qc` — toda interação na UI (incluindo trocar o idioma) reexecuta o pipeline inteiro do zero, para todas as abas (`TODO.md` item 5.4).
